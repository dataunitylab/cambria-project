...
function addProperty(schema: JSONSchema7, property: Property): JSONSchema7 {
  const { properties: origProperties = {}, required: origRequired = [] } =
    schema;
  ...
  const propertyDefinition =
    type === "array" && items
      ? {
          ...
          items: {
            type: items.type,
            default: items.default || defaultValuesByType(items.type),
          },
        }
      : arraylessPropertyDefinition;
  ...
  const shouldAdd =
    isPropertyRequired !== false && !origRequired.includes(name);
...
function withNullable(
  schema: JSONSchema7,
  fn: (s: JSONSchema7) => JSONSchema7,
): JSONSchema7 {
  if (schema.anyOf) {
    if (schema.anyOf.length !== 2) {
      throw new Error(
        "We only support this operation on schemas with one type or a nullable type",
      );
    }
    return {
      ...schema,
...
function supportsNull(schema: JSONSchema7): boolean {
  return (
    ...
    !!schema.anyOf?.some((subSchema) =>
      schemaSupportsType(db(subSchema).type, "null"),
    )
  );
}
...
function findHost(schema: JSONSchema7, name: string): JSONSchema7 {
  if (schema.anyOf) {
    const maybeSchema = schema.anyOf?.find(
      (t) => typeof t === "object" && t.properties,
    );
    if (
      typeof maybeSchema === "object" &&
      typeof maybeSchema.properties === "object"
    ) {
...
function inSchema(schema: JSONSchema7, op: LensIn): JSONSchema7 {
  ...
  if (!name) {
    throw new Error(
      `Expected to find property ${name} in ${Object.keys(op || {})}`,
    );
  }
  ...
  if (host === undefined) {
    throw new Error(
      `Expected to find property ${name} in ${Object.keys(properties || {})}`,
    );
  }
...
function validateSchemaItems(items: JSONSchema7Items) {
  ...
  if (!items || items === true) {
    throw new Error(
      `Cambria requires a specific items definition, found ${items}.`,
    );
  }
  return items;
}
...
function mapSchema(schema: JSONSchema7, lens: LensSource) {
  ...
  if (!schema.items) {
    throw new Error(
      `Map requires a schema with items to map over, ${deepInspect(schema)}`,
    );
  }
  return {
    ...schema,
    items: updateSchema(validateSchemaItems(schema.items), lens),
  };
}
...
function wrapProperty(schema: JSONSchema7, op: WrapProperty): JSONSchema7 {
  if (!op.name) {
    throw new Error(
      "Wrap property requires a `name` to identify what to wrap.",
    );
  }
  ...
  if (!prop) {
    throw new Error(
      `Cannot wrap property '${op.name}' because it does not exist.`,
    );
  }
...
function headProperty(schema, op: HeadProperty) {
  if (!op.name) {
    throw new Error(
      "Head requires a `name` to identify what to take head from.",
    );
  }
  if (!schema.properties[op.name]) {
    throw new Error(
      `Cannot head property '${op.name}' because it does not exist.`,
    );
  }
  ...
  return {
    ...
    properties: {
      ...schema.properties,
      [op.name]: {
        anyOf: [{ type: "null" }, schema.properties[op.name].items],
      },
    },
  };
}
...
function hoistProperty(
  ...
  return withNullable(_schema, (schema) => {
    ...
    const hoistedPropertySchema = withNullable(
      db(properties[host]),
      (hostSchema) => {
...
function plungeProperty(schema: JSONSchema7, host: string, name: string) {
  ...
  if (!destinationTypeProperties) {
    throw new Error(
      `Could not find a property called ${name} among ${Object.keys(properties)}`,
    );
  }
...
function convertValue(schema: JSONSchema7, lensOp: ConvertValue) {
  ...
  if (!name) {
    throw new Error(
      `Missing property name in 'convert'.\nFound:\n${JSON.stringify(lensOp)}`,
    );
  }
  if (!mapping) {
    throw new Error(
      `Missing mapping for 'convert'.\nFound:\n${JSON.stringify(lensOp)}`,
    );
  }
