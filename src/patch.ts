...
export function compile(lensSource: LensSource): {
  right: CompiledLens;
  left: CompiledLens;
} {
  return {
...
export function applyLensToPatchOp(
  ...
  return lensSource.reduce<MaybePatchOp>(
    (prevPatch: MaybePatchOp, lensOp: LensOp) => {
...
function runLensOp(lensOp: LensOp, patchOp: MaybePatchOp): MaybePatchOp {
  ...
  switch (lensOp.op) {
    ...
    case "hoist": {
      ...
      if (
        possibleSource === lensOp.host &&
        possibleDestination === lensOp.name
      ) {
    ...
    case "head": {
      ...
      const headMatch = patchOp.path.match(
        new RegExp(`^/${lensOp.name}/0(.*)`),
      );
