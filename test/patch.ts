...
describe("field rename", () => {
  it("converts upwards", () => {
    ...
    assert.deepEqual(
      applyLensToPatch(lensSource, editTitleV1, projectV1Schema),
  ...
  it("does not rename another property that starts with same string", () => {
    ...
    assert.deepEqual(
      applyLensToPatch(lensSource, editTitleBla, projectV1Schema),
      editTitleBla,
    );
  });
  ...
  it("works with whole doc conversion too", () => {
    ...
    assert.deepEqual(
      applyLensToDoc(lensSource, { title: "hello" }, projectV1Schema),
      {
        complete: "",
        description: "",
        name: "hello",
...
describe("value conversion", () => {
  it("converts from a boolean to a string enum", () => {
    ...
    assert.deepEqual(
      applyLensToPatch(lensSource, setComplete, projectV1Schema),
...
describe("nested objects", () => {
  describe("singly nested object", () => {
    ...
    const lensSource: LensSource = [
      inside("metadata", [renameProperty("title", "name")]),
    ];
    ...
    it("renames a field correctly", () => {
      ...
      assert.deepEqual(
        applyLensToPatch(lensSource, setDescription, docSchema),
    ...
    it("works with whole doc conversion", () => {
      assert.deepEqual(
        applyLensToDoc(lensSource, { metadata: { title: "hello" } }, docSchema),
        {
          metadata: { name: "hello" },
          otherparent: { title: "" },
        },
    ...
    it("doesn't rename another field", () => {
      ...
      assert.deepEqual(
        applyLensToPatch(lensSource, randomPatch, docSchema),
        randomPatch,
      );
    });
...
describe("arrays", () => {
  ...
  const lensSource: LensSource = [
    inside("tasks", [map([renameProperty("title", "name")])]),
  ];
...
describe("default value initialization", () => {
  ...
  const v1Lens: LensSource = [
    ...
    inside("metadata", [
      ...
      inside("flags", [
        addProperty({ name: "O_CREATE", type: "boolean", default: true }),
      ]),
    ]),
    addProperty({
      name: "assignee",
...
describe("inferring schemas from documents", () => {
  ...
  it("infers a schema when converting a doc", () => {
    const lens = [
      inside("details", [renameProperty("height", "heightInches")]),
    ];
