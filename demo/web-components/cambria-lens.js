...
class CambriaLens extends HTMLPreElement {
  constructor() {
    ...
    this.addEventListener("doc-patch", (e) =>
      this.handleDocPatch(e, this.compiledLens),
    );
    this.addEventListener("doc-change", (e) =>
      this.handleDocChange(e, this.compiledLens),
    );
  }
  ...
  translateChange(detail) {
    ...
    const outSchema = Cambria.updateSchema(
      schema,
  ...
  handleDocPatch(event, lens) {
    try {
      ...
      if (!this.schema) {
        throw new Error(
          "Tried to convert a patch before receiving the schema.",
        );
      }
